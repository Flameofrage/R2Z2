  ...
  class R2Z2Twitch
    ...
    def AddStreamer(server)
      unless STREAM_DATA.stream_data[server]["streamers"].include? @username
        ...
        m = STREAM_DATA.stream_data[server].merge!(new_streamer) { |_key, left, right| left.merge!(right) }
        STREAM_DATA.update(m)
        unless STREAMER_HASH.streamer_hash.include? @username
          n = STREAMER_HASH.streamer_hash.merge!(new_streamer['streamers']) { |_key, left, right| left.merge!(right) }
